...
**⚠️ IMPORTANT :** OAuth nécessite une configuration préalable dans le fichier `.env`. Si vous obtenez l'erreur "Unknown authentication strategy", cela signifie que les variables d'environnement OAuth ne sont pas configurées.
...
**Note :** Vous devez créer des applications OAuth sur Google Cloud Console ou GitHub Developer Settings pour obtenir ces identifiants.
...
**⚠️ Si vous obtenez "Unknown authentication strategy \"google\"" :**
...
- Si vous ne souhaitez pas configurer OAuth, vous pouvez ignorer cette section
...
**⚠️ Si vous obtenez "Unknown authentication strategy \"github\"" :**
- Vérifiez que `GITHUB_CLIENT_ID` et `GITHUB_CLIENT_SECRET` sont définis dans `.env`
- Redémarrez le serveur après avoir ajouté les variables d'environnement
- Si vous ne souhaitez pas configurer OAuth, vous pouvez ignorer cette section
...
### Erreur "Unknown authentication strategy \"google\"" ou "Unknown authentication strategy \"github\""
- **Cause :** Les variables d'environnement OAuth ne sont pas configurées dans le fichier `.env`
- **Explication :** Les stratégies Passport ne sont enregistrées que si les identifiants OAuth sont présents dans `.env`
- **Solutions :**
  1. **Si vous voulez utiliser OAuth :**
     ...
     - Vous devez créer une application OAuth sur Google Cloud Console ou GitHub Developer Settings
  2. **Si vous ne voulez pas utiliser OAuth (recommandé pour les tests) :**
     - Ignorez simplement cette section du guide
     - OAuth est optionnel pour le TP
     - Toutes les autres fonctionnalités fonctionnent sans OAuth
